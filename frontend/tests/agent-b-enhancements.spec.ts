...
test.describe('Agent-A: 协作增强功能验证', () => {
  ...
  test('Test-B4: 用户加入/离开通知', async ({ browser }) => {
    ...
    await expect(pageA.getByText('在线: 2')).toBeVisible({ timeout: 15000 })
    ...
    await expect(pageA.getByText('在线: 3')).toBeVisible({ timeout: 15000 })
  ...
  test('Test-A2/A3: 格式应用验证', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const quill = (window as any).__quill
      if (!quill) return
